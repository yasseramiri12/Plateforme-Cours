...
export default function FilieresPage() {
    ...
    const handleDelete = async (id) => {
        const token = localStorage.getItem('auth_token');
        try {
            const previousFilieres = [...filieres];
            setFilieres(filieres.filter(f => f.id_filiere !== id));

            await axios.delete(`http://localhost:8000/api/admin/filieres/${id}`, { 
                headers: { Authorization: `Bearer ${token}` } 
            });
            const filiereName = previousFilieres.find(f => f.id_filiere === id)?.nom_filiere;
            showToast(`Filière ${filiereName || ''} supprimée`, 'warning');
        } catch (e) { 
            fetchFilieres(); 
    ...
    // 3. Créer ou modifier une filière
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                // Modification
                await axios.put(`http://localhost:8000/api/admin/filieres/${editingId}`, formData, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                showToast('Filière modifiée avec succès !', 'success');
            } else {
                // Création
                await axios.post('http://localhost:8000/api/admin/filieres', formData, {
                    headers: { Authorization: `Bearer ${token}` }
                });
    ...
    const resetForm = () => {
        setFormData({ nom_filiere: '', code_filiere: '', description: '' });
        setEditingId(null);
    ...
    const handleEdit = (filiere) => {
        ...
        setEditingId(filiere.id_filiere);
        setIsModalOpen(true);
    };
    ...
    return (
        <AdminLayout title="Gestion des Filières">
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    <th className="px-6 py-3.5 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Nom</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {filteredFilieres.length === 0 ? (
                                    ...
                                    <tr key={f.id_filiere} className="hover:bg-gray-50 transition-colors group">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                                <div className="h-10 w-10 rounded-full bg-gradient-to-tr from-emerald-500 to-teal-600 flex items-center justify-center text-white font-bold text-sm mr-4 shadow-sm group-hover:shadow-md transition-shadow">
                                                    {(f.nom_filiere || '').charAt(0).toUpperCase()}
                                                </div>
                                                <div className="text-sm font-semibold text-gray-900">{f.nom_filiere}</div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td className="px-6 py-4">
                                            <p className="text-sm text-gray-600 line-clamp-2">
                                                {f.description || <span className="italic text-gray-400">Aucune description</span>}
                                            </p>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex gap-2 justify-end">
                                            ...
                                            <button 
                                                onClick={() => handleDelete(f.id_filiere)} 
                            ...
                            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <div className="flex justify-between items-center mb-5 border-b border-gray-100 pb-3">
                                    <h3 className="text-lg leading-6 font-bold text-gray-900" id="modal-title">
                                        {editingId ? 'Modifier la Filière' : 'Créer une nouvelle filière'}
                                    </h3>
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    <div className="grid grid-cols-3 gap-4">
                                        <div className="col-span-2">
                                            ...
                                            <input required type="text" name="nom_filiere" value={formData.nom_filiere} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm border p-2.5 transition-shadow" placeholder="ex. Informatique" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Code</label>
                                            <input required type="text" name="code_filiere" value={formData.code_filiere} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm border p-2.5 transition-shadow" placeholder="ex. INF" />
                                        </div>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
                                        <textarea rows="3" name="description" value={formData.description} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm border p-2.5 transition-shadow resize-none" placeholder="Brève description des objectifs..." />
                                    </div>
                                    ...
                                    <div className="pt-4 flex items-center justify-end gap-3 border-t border-gray-100 mt-6">
                                        ...
                                        >
                                            {submitting ? (
                                                ...
                                                <span className="flex items-center"><Check className="-ml-1 mr-2 h-4 w-4" /> {editingId ? 'Modifier' : 'Créer la filière'}</span>
                                            )}
                                        </button>
                                    </div>
                                </form>
