...
export default function DocumentsPage() {
    ...
    const fetchDocuments = async () => {
        ...
        try {
            const res = await axios.get('http://localhost:8000/api/admin/documents', { 
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            setDocuments(documents.filter(d => d.id_document !== id && d.id_cours !== id));

            await axios.delete(`http://localhost:8000/api/admin/documents/${id}`, { 
                headers: { Authorization: `Bearer ${token}` } 
            });
            const docName = previousDocuments.find(d => d.id_document === id || d.id_cours === id)?.titre;
            showToast(`Document ${docName || ''} supprimé`, 'warning');
        } catch (e) { 
            fetchDocuments(); 
    ...
    // 3. Créer ou modifier un document
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                // Modification
                await axios.put(`http://localhost:8000/api/admin/documents/${editingId}`, formData, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                showToast('Document modifié avec succès !', 'success');
            } else {
                // Création
                await axios.post('http://localhost:8000/api/admin/documents', formData, {
                    headers: { Authorization: `Bearer ${token}` }
                });
    ...
    const resetForm = () => {
        setFormData({ titre: '', description: '', fichier_url: '', type_document: 'COURS' });
        setEditingId(null);
    ...
    const handleEdit = (doc) => {
        ...
        setEditingId(doc.id_document || doc.id_cours);
        setIsModalOpen(true);
    };
    ...
    const filteredDocuments = documents.filter(d => 
        ...
        (d.type_document || '').toLowerCase().includes(search.toLowerCase())
    );

    const getTypeColor = (type) => {
        switch(type) {
            case 'COURS': return 'bg-blue-50 text-blue-700 border-blue-200';
            case 'TP': return 'bg-purple-50 text-purple-700 border-purple-200';
            case 'TD': return 'bg-green-50 text-green-700 border-green-200';
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-6 py-3.5 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">Fichier</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {filteredDocuments.length === 0 ? (
                                    ...
                                    <tr key={d.id_document || d.id_cours} className="hover:bg-gray-50 transition-colors group">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                                <div className="h-10 w-10 rounded-full bg-gradient-to-tr from-cyan-500 to-blue-600 flex items-center justify-center text-white font-bold text-sm mr-4 shadow-sm group-hover:shadow-md transition-shadow">
                                                    <FileText className="w-5 h-5" />
                                                </div>
                                                <div className="text-sm font-semibold text-gray-900">{d.titre}</div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`px-2.5 py-0.5 text-xs font-bold rounded-full border ${getTypeColor(d.type_document)}`}>
                                                {d.type_document}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <p className="text-sm text-gray-600 line-clamp-2">
                                                {d.description || <span className="italic text-gray-400">Aucune description</span>}
                                            </p>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex gap-2 justify-end">
                                            ...
                                            <button 
                                                onClick={() => handleDelete(d.id_document || d.id_cours)} 
                            ...
                            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <div className="flex justify-between items-center mb-5 border-b border-gray-100 pb-3">
                                    <h3 className="text-lg leading-6 font-bold text-gray-900" id="modal-title">
                                        {editingId ? 'Modifier le Document' : 'Créer un nouveau document'}
                                    </h3>
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    <div>
                                        ...
                                        <input required type="text" name="titre" value={formData.titre} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm border p-2.5 transition-shadow" placeholder="ex. Chapitre 1 - Introduction" />
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Type de Document</label>
                                        <select required name="type_document" value={formData.type_document} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm border p-2.5 transition-shadow">
                                    ...
                                    <div>
                                        ...
                                        <textarea rows="3" name="description" value={formData.description} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm border p-2.5 transition-shadow resize-none" placeholder="Brève description du document..." />
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">URL du Fichier</label>
                                        <input type="url" name="fichier_url" value={formData.fichier_url} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm border p-2.5 transition-shadow" placeholder="https://exemple.com/document.pdf" />
                                    </div>
                                    ...
                                    <div className="pt-4 flex items-center justify-end gap-3 border-t border-gray-100 mt-6">
                                        ...
                                        >
                                            {submitting ? (
                                                ...
                                                <span className="flex items-center"><Check className="-ml-1 mr-2 h-4 w-4" /> {editingId ? 'Modifier' : 'Créer le document'}</span>
                                            )}
                                        </button>
                                    </div>
                                </form>
